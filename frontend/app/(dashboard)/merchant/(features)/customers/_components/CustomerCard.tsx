...
export default function CustomerCard({ customer }: CustomerCardProps) {
  ...
  // Assuming inputs are like 010xxxx, 011xxxx
  // Output should be 2010xxxx
  const cleanPhone = displayPhone.replace(/\D/g, '').replace(/^0+/, '');
  const waLink = `https://wa.me/20${cleanPhone}`; 
    ...
    <div className="bg-white border-b border-gray-100 active:bg-blue-50/30 transition-colors p-4 flex items-center justify-between">
       ...
       <div className="flex items-center gap-3 shrink-0 ms-2">
          ...
          <a
             ...
             className="w-11 h-11 flex items-center justify-center rounded-full bg-green-50 text-green-600 border border-green-100 shadow-sm active:scale-95 transition-all"
             aria-label="WhatsApp"
          >
