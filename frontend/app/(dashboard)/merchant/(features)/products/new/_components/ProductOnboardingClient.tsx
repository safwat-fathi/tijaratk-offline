...
import {
  buildAvailableCatalogCategories,
...
export default function ProductOnboardingClient({
  ...
  const [manualOrderMode, setManualOrderMode] = useState<ProductOrderMode>(ORDER_MODE_QUANTITY);
  ...
  const [manualSecondaryUnitMultiplier, setManualSecondaryUnitMultiplier] = useState('');
  const [manualWeightPresets, setManualWeightPresets] = useState(DEFAULT_WEIGHT_PRESETS);
  const [manualPricePresets, setManualPricePresets] = useState(DEFAULT_PRICE_PRESETS);
  ...
  const [editOrderMode, setEditOrderMode] = useState<ProductOrderMode>(ORDER_MODE_QUANTITY);
  ...
  const [editSecondaryUnitMultiplier, setEditSecondaryUnitMultiplier] = useState('');
  const [editWeightPresets, setEditWeightPresets] = useState(DEFAULT_WEIGHT_PRESETS);
  const [editPricePresets, setEditPricePresets] = useState(DEFAULT_PRICE_PRESETS);
  ...
  const availableCatalogCategories = useMemo(
    () => buildAvailableCatalogCategories(catalogCategories),
    [catalogCategories],
  );
    ...
    startTransition(async () => {
      ...
      if (!response.success || !response.data) {
        if (isDuplicateMessage(response.message)) {
          const existingProduct = productsByNormalizedName.get(normalizeProductName(trimmedName));
          if (existingProduct) {
  ...
  const handleAddFromCatalog = (item: CatalogItem) => {
    const duplicateProduct = productsByNormalizedName.get(normalizeProductName(item.name));
    if (duplicateProduct) {
      highlightExistingProduct(duplicateProduct);
      return;
    }
    ...
    void (async () => {
      try {
        ...
        if (!response.success || !response.data) {
          if (isDuplicateMessage(response.message)) {
            const existingProduct = productsByNormalizedName.get(normalizeProductName(item.name));
            if (existingProduct) {
              highlightExistingProduct(existingProduct);
              return;
            }
  ...
  const handleStartEdit = (product: Product) => {
    ...
    const editState = deriveEditFormState(product, availableCatalogCategorySet);
