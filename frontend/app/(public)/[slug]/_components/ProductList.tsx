			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				{products.map(product => {
					...
					return (
						<div
							key={product.id}
							className={`bg-white p-4 rounded-xl border transition-all duration-200 ${qty > 0 ? "border-primary shadow-md ring-1 ring-primary/20" : "border-gray-100 shadow-sm hover:shadow-md"}`}
						>
										...
										<div className="flex items-center gap-3 bg-gray-50 rounded-full p-1 border border-gray-100">
											{qty > 0 && (
												<>
													...
													<span className="w-4 text-center font-bold text-gray-900 select-none">
														{qty}
													</span>
												</>
											)}
